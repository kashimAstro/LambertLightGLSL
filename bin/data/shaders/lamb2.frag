...
C_Sample SampleMaterial(const in vec2 vUV, sampler2D sampler,  const in vec2 vTextureSize, const in float fNormalScale)
{
	...
	result.vAlbedo = ( cLSampleNegXNegY + cLSampleZerXNegY + cLSamplePosXNegY + cLSampleNegXZerY + cLSampleZerXZerY + cLSamplePosXZerY + cLSampleNegXPosY + cLSampleZerXPosY + cLSamplePosXPosY ) / 9.0;	
	...
	#endif
	vec2 vEdge;
	vEdge.x = (fSampleNegXNegY - fSamplePosXNegY) * 0.25 + (fSampleNegXZerY - fSamplePosXZerY) * 0.5 + (fSampleNegXPosY - fSamplePosXPosY) * 0.25;
	vEdge.y = (fSampleNegXNegY - fSampleNegXPosY) * 0.25 + (fSampleZerXNegY - fSampleZerXPosY) * 0.5 + (fSamplePosXNegY - fSamplePosXPosY) * 0.25;
...
{	
	...
	float fNormalScale = 20.0;
	materialSample = SampleMaterial( vUV, u_texture, resImage.xy, fNormalScale );
	
	float fLightHeight = 0.08;
	float fViewHeight = 0.9;
	
	vec3 vSurfacePos = vec3(vUV, 0.0);
	vec3 vViewPos = vec3(0.5, 0.5, fViewHeight);
	vec3 vLightPos = vec3(LightPos.x,LightPos.y,fLightHeight);
	...
	vec3 vResult = materialSample.vAlbedo * fDiffuse + fSpec * sColor;
